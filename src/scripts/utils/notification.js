...
import { pushSubscribe, pushUnsubscribe  } from '../data/api';
...
export async function subscribe() {
  ...
  try {
    ...
    const response = await pushSubscribe({ endpoint, keys });
...
export async function unsubscribe() {
  ...
  try {
    ...
    const { endpoint, keys } = pushSubscription.toJSON();
    const response = await pushUnsubscribe({ endpoint });
    if (!response.ok) {
    ...
    if (!unsubscribed) {
      alert(failureUnsubscribeMessage);
      await pushSubscribe({ endpoint, keys });
      return;
    }
